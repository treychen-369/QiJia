...
import { Plus, Search, ArrowDown, ArrowUp, TrendingUp, TrendingDown, MoreHorizontal, Edit, Trash2, Coins, Zap, Package, Gem, Car, ChevronDown, ChevronUp } from 'lucide-react';
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
export function AlternativeList({ 
  ...
  const [sortBy, setSortBy] = useState<'value' | 'pnl' | 'yield' | 'quantity'>('value');
  ...
  const filteredAssets = assets.filter(asset =>
    ...
    (asset.metadata?.symbol && asset.metadata.symbol.toLowerCase().includes(searchTerm.toLowerCase()))
  );
  ...
  const groupInfo = {
    'ALT_GOLD': { name: '贵金属', icon: Coins, color: 'bg-yellow-500' },
    'ALT_CRYPTO': { name: '数字资产', icon: Zap, color: 'bg-purple-500' },
    'ALT_COMMODITY': { name: '大宗商品', icon: Package, color: 'bg-orange-500' },
    'ALT_COLLECTIBLE': { name: '收藏品', icon: Gem, color: 'bg-pink-500' },
    'ALT_PHYSICAL': { name: '实物资产', icon: Car, color: 'bg-slate-500' },
    ...
    <Card className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800 border-0 shadow-xl">
      ...
      <CardContent className="pt-0">
        ...
        <div className="flex items-center gap-1.5 mb-2">
          ...
          <Badge className="bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300 text-[10px] h-5 px-1.5">
        ...
        {filteredAssets.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12">
            ...
            <h3 className="text-lg font-semibold mb-2">暂无另类投资</h3>
            <p className="text-sm text-muted-foreground mb-4">
              {searchTerm ? '没有找到匹配的资产' : '点击上方按钮添加您的第一笔另类投资'}
            </p>
            {!searchTerm && (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <Badge variant="outline" className="text-[10px] h-5 px-1.5 shrink-0">
                          {asset.metadata?.symbol || asset.currency}
                        </Badge>
                      </div>
                      
                      {/* 第二行：数量和购买日期 */}
                      <div className="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-400 mb-3">
                        <span className="shrink-0">数量 {formatters.number(asset.quantity, 4)}</span>
                        {asset.metadata?.currentPrice && (
                          <span className="shrink-0">单价 {asset.currency}{Number(asset.metadata.currentPrice).toLocaleString('zh-CN')}</span>
                        )}
                        {asset.purchaseDate && (
                          <span className="text-slate-400 shrink-0">{new Date(asset.purchaseDate).toLocaleDateString('zh-CN')}</span>
                        )}
                      </div>
                      ...
                      <div className="flex items-end justify-between pt-2 border-t border-slate-100 dark:border-slate-700">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            ...
                            <span className={`text-xs ${getPnLColorClass(asset.unrealizedPnlPercent || 0, preferences.colorScheme)}`}>
